...
export function ImagesPanel() {
  ...
  const reset = async (chave: string) => {
    const existing = rows.find((r) => r.chave === chave);
    if (!existing) return;
    if (!confirm("Excluir esta imagem? A área ficará sem imagem no site até que outra seja enviada.")) return;
  ...
  return (
    <div>
      ...
      <p className="text-sm text-muted-foreground">Cada espaço abaixo indica onde a imagem aparece no site. Sem imagem enviada, aquela área simplesmente não exibe foto.</p>
      <div className="mt-6 grid gap-5 md:grid-cols-2">
        {IMAGE_SLOTS.map((slot) => {
          const row = rows.find((r) => r.chave === slot.key);
          const preview = row ? urls[row.url] : null;
          return (
            <div key={slot.key} className="rounded-2xl border bg-card p-4">
              <p className="text-sm font-semibold">{slot.rotulo}</p>
              <p className="text-xs text-muted-foreground">{row ? "Imagem enviada pela equipe" : "Nenhuma imagem cadastrada"}</p>
              {preview ? (
                <img src={preview} alt={slot.rotulo} className="mt-3 aspect-video w-full rounded-xl object-cover" />
              ) : (
                <div className="mt-3 grid aspect-video w-full place-items-center rounded-xl border border-dashed text-xs text-muted-foreground">Sem imagem</div>
              )}
              <div className="mt-3 flex flex-wrap gap-2">
                ...
                {row && <button onClick={() => void reset(slot.key)} className="rounded-full border border-destructive/30 px-4 py-2 text-sm font-semibold text-destructive hover:bg-destructive/10"><Trash2 className="mr-1 inline size-4" />Excluir</button>}
              </div>
            </div>
          );
        })}
